...
func move_part(part_pos: Vector2i):
	...
	if (part != null):
		if (part_pos.y > 0 and not parts.has(part_pos - Vector2i(0, 1))):
				...
				smooth_update_part(part, part_pos - Vector2i(0, 1), part_pos)
				...
				smooth_update_part(part, part_pos - Vector2i(1, 0), part_pos)
...
func smooth_update_part(part : Control, pos : Vector2i, old_pos : Vector2i):
	part.target_pos = Rect2(float(pos.x) / float(CELLS_COUNT.x), float(pos.y) / float(CELLS_COUNT.y), float(pos.x + 1) / float(CELLS_COUNT.x), float(pos.y + 1) / float(CELLS_COUNT.y))
	#if (old_pos.y != pos.y):
